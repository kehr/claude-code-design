...
> 完整 PlantUML 源文件: [diagrams/tool-permission-flow.puml](diagrams/tool-permission-flow.puml)
...
每个工具通过 `checkPermissions()` 方法声明所需权限。权限检查流程详见 [diagrams/tool-permission-flow.puml](diagrams/tool-permission-flow.puml)。
